    ...
    for intent in tqdm.tqdm(total_intent):
        ...
        for subset_idx, subset in enumerate(query_info['subset']):
            ...
            if args.grouping_method == 'roberta_soft':
                ...
                for i in range(len(now_d_score)):
                    now_d_score[i] = scaler.transform([[now_d_score[i]]])[0][0]
            ...
            for rand in range(20): # 100
                ...
                for d in now_d_list:
                    ...
                    if args.grouping_method == 'roberta_soft':
                        ...
                        info_list[0] = simulator.calc_score_anno2(600, truth)
                    else:
                        truth = int(intent) in anno[q][str(d)]['anno']
                        info_list[0] = simulator.calc_score_anno(600, truth)
                        jie = 0
                ...
                for eeg_gamma in [5]:
                    for click_gamma in [1.5,2]:
                        ...
                        for alpha in [1.0,]:
                            ...
                            re = ndcg_score([now_d_score], [combine_array(future_score2, future_score1, args.alpha)], k=10)
                            if args.click == 'add':
                            ...
                            intent2subset2para2performance[intent][subset_idx][para].append(re)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-task',type=str, required=False, default = 'rdr')
    ...
    json.dump(result_list, open(f'../results/simulating/rdr_result_list.{args.mode}.{args.grouping_method}.json','w'))
    json.dump(true_data, open(f'../results/simulating/rdr.{args.mode}.{args.grouping_method}.json','w'))
